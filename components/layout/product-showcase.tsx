      ...
      {products.map((product) => (
        <Grid.Item key={product.handle} className="border rounded-lg p-4">
          ...
          >
            ...
            <h2 className='text-sm text-muted-foreground line-through text-gray-600'>
              <s>R$ {product.variants[0]?.compareAtPrice.amount ? parseFloat(product.variants[0].compareAtPrice.amount).toFixed(2).replace('.', ',') : 'N/A'}</s>
            </h2>
            ...
            <h3 className='text-4xl font-semibold text-gray-950'>
              R$ {product.variants[0] && product.variants[0].price.amount ? (parseFloat(product.variants[0].price.amount) / 6).toFixed(2).replace('.', ',') : 'N/A'}
            </h3>
            <h3 className='text-lg font-semibold text-gray-600'>
              Ou <strong className='text-gray-950'>R$ {product.variants[0]?.price.amount ? parseFloat(product.variants[0].price.amount).toFixed(2).replace('.', ',') : 'N/A'}</strong> à vista
            </h3>
